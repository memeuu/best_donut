...
async function getTodayIns(){
  const response = await fetch("/ajax/todayProdIns");
...
async function getProcMatInfo(prdCode){
  const response = await fetch(`/ajax/procMatInfo?procDetailCode=${prdCode}`);
  const res = await response.json();
...
async function startProc() {
  ...
  const response = await fetch('ajax/updateProc', {
    method: 'post',
    headers: jsonHeaders,
